...
def main():
    ...
    if len(file_list) > 0:
        ...
        for fil in file_list:
            ...
            # Process entire file
            fil_dir, fil_filename = os.path.split(fil)
            # print(f"absolute template dir {abs_templ_dir} and filename {abs_templ_filename}")
            fil_name, fil_ext = os.path.splitext(fil_filename)
            hostname = get_hostname_from_filename(fil_name)
            print(hostname)
            ...
            if len(fil_results) == 0:
                ...
                for line in fil_results:
                    print(line)
                    fil_results_hn.append([[hostname] + line])

                fsm_all_results.append(fil_results)

            fsm_all_res_dict.update({hostname: fil_results})

        # for k,v in fsm_all_res_dict.items():
        #     print(f"\n{k}")
        #     for line in v:
        #         print(line)
        #
        # print(f"\n\n")
        # for line in fsm_all_results[0]:
        #     print(line)
        #     print()
